...
(function () {
	...
	class DOMToolkit {
		...
		query(selector, options = {}) {
			...
			// 尝试从缓存获取（仅单元素查询）
			if (!all && useCache && selectors.length === 1) {
			...
			for (const sel of selectors) {
				try {
					if (all) {
						const results = Array.from(parent.querySelectorAll(sel));
						if (shadow) {
							this.#collectInShadow(parent, sel, results, 0, maxDepth);
						}
						if (results.length > 0) return results;
					} else {
						const el = parent.querySelector(sel);
						if (el) {
							if (useCache) this.#cache.set(parent, sel, el);
							return el;
						}
					}
				} catch (e) {
			...
			if (shadow && !all) {
				const found = this.#findInShadow(parent, selectors, 0, maxDepth);
				if (found && useCache && selectors.length === 1) {
		 ...
		 * @private
		 */
		#findInShadow(root, selectors, depth, maxDepth) {
			...
			for (const el of elements) {
				if (el.shadowRoot) {
					const found = this.#findInShadow(el.shadowRoot, selectors, depth + 1, maxDepth);
		 ...
		 * @private
		 */
		#collectInShadow(root, selector, results, depth, maxDepth) {
			...
			for (const el of elements) {
				if (el.shadowRoot) {
					this.#collectInShadow(el.shadowRoot, selector, results, depth + 1, maxDepth);
				}
			}
		}
		...
		async get(selector, options = {}) {
			...
			// 先尝试同步查找
			const found = this.query(selector, { parent, shadow });
			if (found) return found;
			...
			return new Promise((resolve) => {
				...
				const poll = () => {
					...
					const result = this.query(selector, { parent, shadow });
					if (result) {
						cleanup.execute();
						resolve(result);
						return;
				...
				const callback = (addedNode) => {
					for (const sel of selectors) {
						try {
							...
							const found = addedNode.querySelector && addedNode.querySelector(sel);
							if (found) {
								cleanup.execute();
								resolve(found);
								return;
	...
	console.log('[DOMToolkit] v1.0.0 Loaded');
})();
