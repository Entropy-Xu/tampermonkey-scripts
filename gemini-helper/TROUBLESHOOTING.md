...
7. [Flutter 图文并茂滚动失效](#7-flutter-图文并茂滚动失效)
...
- 在 Gemini Business 的「图文并茂模式」中，点击「去底部」按钮只滚动几像素，无法真正到达底部
- 「去顶部」按钮同样无效
- 「返回锚点」跳转到错误位置
...
Gemini 的「图文并茂模式」使用 Flutter 渲染引擎，内容通过 `<iframe>` 嵌入。滚动容器是 `<flt-semantics>` 元素，而非普通的 DOM 元素。
