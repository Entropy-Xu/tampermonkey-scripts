...
- **解决 Flutter 图文并茂滚动问题**：在「图文并茂模式」中，「去顶部」「去底部」「返回锚点」按钮现在能正常工作
  - 扩展滚动容器查找逻辑，支持穿透 iframe 边界查找 Flutter 的 `flt-semantics` 元素
  - 使用循环滚动机制处理 `transform: scale()` 导致的 scrollHeight 失真问题
  - 缓存 Flutter 滚动容器引用，避免重复查找导致的不稳定问题
- **解决面板在图文并茂中重复加载问题**
- **优化边缘吸附交互**：从吸附状态临时显示面板后，鼠标移出时自动恢复吸附
